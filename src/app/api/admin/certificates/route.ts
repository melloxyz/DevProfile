...
export async function GET(): Promise<NextResponse> {
  ...
  return NextResponse.json(snapshot.certificates, {
    status: 200,
    headers: {
      "Cache-Control": "no-store",
    },
...
export async function PUT(request: NextRequest): Promise<NextResponse> {
  ...
  const items = parseCertificatesInput(pickItemsPayload(body));

  if (!items) {
    return badRequestResponse("Lista de certificados invalida.");
  }
